...
export LIMIT_MODE="${LIMIT_MODE:-per_shard}"       # per_shard | global
...
export EXPLAIN_SPAN_MODE="${EXPLAIN_SPAN_MODE:-answer}"
export EAGLE_TOKEN_MODE="${EAGLE_TOKEN_MODE:-span}"
export EAGLE_TOKEN_LIMIT="${EAGLE_TOKEN_LIMIT:-0}"
export SAVE_EAGLE_ARTIFACTS="${SAVE_EAGLE_ARTIFACTS:-0}"
...
export WORKERS_PER_GPU="${WORKERS_PER_GPU:-1}"     # duplicate workers per listed GPU; prefer LIMIT_MODE=global
export ATTN="${ATTN:-}"                            # empty lets eagle_g0.sh choose eager/sdpa from GRAD_PROBES
