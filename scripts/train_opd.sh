...
# Full FT, global batch 512 (Vision-OPD/VGS Table 4). Default per_device 8 x
# grad_accum 8 x 8 GPU = 512 (per_device 8 batches the vLLM rollout — faster). If
# OOM on long rollouts, drop to PER_DEVICE_TRAIN_BATCH_SIZE=4 GRADIENT_ACCUMULATION_STEPS=16
# (or pd=2 ga=32; still 512). Rescale grad_accum to keep eff_batch 512 if GPU count changes.
