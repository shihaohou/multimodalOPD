...
@dataclass
class GlimpseResult:
    iou_gl: float  # thresholded visual map vs GT grid mask
    bbox_iou: float
    pointing: float
    energy: float  # positive visual mass in GT box
    vt_ratio: float  # V / (V + T_prompt)
    visual_mass: float
    textual_mass: float
...
def _layer_response_relevance(
    ...
    """One layer's β-weighted response-row relevance ``[S]`` and grad-norm ``g_ℓ``.
    ...
    instead of the full ``[S,S]`` matrix.
    """
    relu_g = torch.relu(g)
...
def glimpse_relevance(
    ...
    *,
    ...
    """Compute the GLIMPSE relevance over all sequence positions ``[S]`` (numpy).

    Returns ``(relevance, info)`` where ``relevance[i] ≥ 0`` is how much input
    position ``i`` drives the response, and ``info`` carries the position masks.
    ...
    *values* in ``out.attentions`` remain readable for LH.
    ...
    # Eq.17 confidence weight (detached — it weights, it is not differentiated).
    with torch.no_grad():
        probs = torch.softmax(sel.float(), dim=-1)
        p_t = probs[row_idx, targets].clamp_min(1e-12)
        beta = (p_t / p_t.sum()).float()  # [R]
    # Backward scalar S = Σ_t z_t (raw logit of the generated token).
    ...
    for gl, l in zip(grads, layer_ids):
        ...
        rel, gnorm = _layer_response_relevance(g, a, response_rows, beta, lam)
    ...
    for a_l, rel in zip(alpha.tolist(), rel_layers):
        relevance += a_l * rel.double().cpu()
    relevance = torch.relu(relevance).numpy()  # non-negative attribution
...
def glimpse_probe(
    ...
    *,
    ...
    """Full GLIMPSE probe for one sample/condition: visual IoU + vt_ratio.
    ...
    relevance, _ = glimpse_relevance(
        gm, inputs, full_ids, prompt_len, completion_ids,
        layers=layers, lam=lam, lambda_depth=lambda_depth, out=out,
    )

    vis_idx = visual_positions.detach().cpu().numpy()
    visual_vec = relevance[vis_idx]
    visual_map = visual_vec.reshape(h_grid, w_grid)

    # Modality split. Textual prompt = prompt positions that are NOT image tokens;
    # self = the response's own positions (autoregressive "prior").
    prompt_mask = np.zeros(relevance.shape[0], dtype=bool)
    prompt_mask[:prompt_len] = True
    visual_mask = np.zeros_like(prompt_mask)
    visual_mask[vis_idx] = True
    textual_mask = prompt_mask & ~visual_mask
    self_mask = ~prompt_mask

    visual_mass = float(relevance[visual_mask].sum())
    textual_mass = float(relevance[textual_mask].sum())
    self_mass = float(relevance[self_mask].sum())
    denom = visual_mass + textual_mass
    vt_ratio = visual_mass / denom if denom > 0 else float("nan")
